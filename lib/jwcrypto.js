...
// for symmetric keys, size is in bits (must be a multiple of 32)
exports.generateKey = function(size) {
  var rawKey = libs.sjcl.random.randomWords(size / 32); // wants the number of 4-byte words
...
// data is the raw entropy (integer or string) and entropy is the number of bits of entropy it contains
exports.addEntropy = function(data, entropy) {
  libs.sjcl.random.addEntropy(data, entropy, 'unknown');
};

exports.pbkdf2 = function(password, salt) {
  return JSON.stringify(libs.sjcl.misc.pbkdf2(password, salt));
};
