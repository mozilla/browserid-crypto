...
var verifyChain = function(certs, now, getRoot, cb) {
  if (!certs.length)
    return cb("certs must be an array of at least one cert");
  ...
  function verifyCert(i, pk, certParamsArray, cb) {
    ...
    verify(certs[i], pk, now, function(err, payload, assertionParams, certParams) {
      ...
      if (i >= certs.length)
        ...
        verifyCert(i, certParams['public-key'], certParamsArray, cb);
    });
  }
...
exports.verifyBundle = function(bundle, now, getRoot, cb) {
  ...
  if (typeof(bundle) != 'string')
    return cb("malformed backed assertion");
  ...
  // no certs? not okay
  if (certs.length == 0)
    return cb("no certificates provided");
